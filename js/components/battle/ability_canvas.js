define([
	...
	var Model = Canvas.Model.extend({
		...
		filterManifest : function(pre){
			...
			if(_.has(avatars_special_weapon, json.user.avatar_id)){
				var special_weapons = avatars_special_weapon[json.user.avatar_id];
				post = post.map(function(m){
					var obj = _.extend({},m);
					special_weapons.forEach(function(weapon_id){
						if(obj.id == weapon_id){
							var filename = weapon_id + '_id' + json.user.avatar_id + '.png';
			...
			var regex = new RegExp('^id' + json.boy_avatar_id + '(.+)$');
			post = post.map(function(m){
				var obj = _.extend({},m);
				var result = obj.id.match(regex);
				if(result != null){
					var filename = 'id' + json.user.avatar_id + result[1] +'.png';
					obj.src = m.src.replace(/\/[^\/]+$/, '/' + filename);
				}
				return obj;
			});
